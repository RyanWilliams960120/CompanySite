...
async function ensureSchema(sql) {
  ...
  await sql`ALTER TABLE applications DROP CONSTRAINT IF EXISTS applications_resume_size_check`;
  await sql`ALTER TABLE applications ADD CONSTRAINT applications_resume_size_check CHECK (resume_size > 0 AND resume_size <= 4194304)`;
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS contact_messages_created_at_idx ON contact_messages (created_at DESC)`;
